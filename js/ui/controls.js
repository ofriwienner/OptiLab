...
const COLOR_PRESETS = [
    '#ef4444', '#fb923c', '#fbbf24', '#22c55e',
    '#0ea5e9', '#3b82f6', '#6366f1', '#a855f7', '#ec4899', '#e2e8f0',
];
...
function tryAutoAlign(element) {
    ...
    if (candidates.length > 0) {
        ...
        } else if (element.type === 'fiber-coupler' && !element.pairedWith) {
...
function toggleBoardUnit() {
    ...
    if (boardUnitIsInches) {
        wInput.value = (Math.round(wMm / 25.4)).toFixed(0);
        hInput.value = (Math.round(hMm / 25.4)).toFixed(0);
        wInput.step = '0.25';
        hInput.step = '0.25';
        btn.textContent = 'in';
    } else {
...
function updateBoardInputs() {
    ...
    if (s.value !== 'custom') {
        ...
        if (boardUnitIsInches) {
            document.getElementById('boardW').value = (Math.round(parseInt(w) / 25.4)).toFixed(0);
            document.getElementById('boardH').value = (Math.round(parseInt(h) / 25.4)).toFixed(0);
        } else {
